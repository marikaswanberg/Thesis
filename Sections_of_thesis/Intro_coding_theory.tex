...
\begin{definition}[Message, encoding] We will denote our message $m$ as $m_0m_1m_2 \ldots m_{k-1}$ where $m_i \in \{0,1\} \text{ for all }0 \leq i \leq k-1$. Next, we will denote the encoding of our message $m$, as determined by an encoding algorithm, $E(m) = u_0u_1u_2 \ldots u_{n-1}$ where $n > k$ and $u_i \in \{0,1\} \text{ for all }0 \leq i \leq n-1$. 
\end{definition}
